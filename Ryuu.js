...
const fakeu = fs.readFileSync ('./media/thumb.jpg')
...
module.exports = u = async (u, mek, _welkom) => {
	try {
		...
		const botNumber = u.user.jid
		const ownerNumber = [`${owner}@s.whatsapp.net`, `6281220582411@s.whatsapp.net`]
		const isGroup = from.endsWith('@g.us')		
		const isQuotedReply = type === 'extendedTextMessage' && content.includes('Message')
		let sender = isGroup ? mek.participant : mek.key.remoteJid
		let senderr = mek.key.fromMe ? u.user.jid : mek.key.remoteJid.endsWith('@g.us') ? mek.participant : mek.key.remoteJid
		const groupMetadata = isGroup ? await u.groupMetadata(from) : ''
		...
		const conts = mek.key.fromMe ? u.user.jid : u.contacts[sender] || { notify: jid.replace(/@.+/, '') }
        const pushname = mek.key.fromMe ? u.user.name : conts.notify || conts.vname || conts.name || '-'    
		...
		let bio_nya = await u.getStatus(sender)
		try {
...
Badmin: 'Bot Harus Jadi Admin Kalo Mau Menggunakan Fiturnya', 
        ...
        const reply = (teks) => {
            u.sendMessage(from, teks, text, {quoted:mek})
        }
        const sendMess = (hehe, teks) => {
            u.sendMessage(hehe, teks, text)
        }
        const mentions = (teks, memberr, id) => {
            (id == null || id == undefined || id == false) ? u.sendMessage(from, teks.trim(), extendedText, { contextInfo: { "mentionedJid": memberr } }) : u.sendMessage(from, teks.trim(), extendedText, { quoted: ftrol, contextInfo: { "mentionedJid": memberr } })
        }
        const costum = (pesan, tipe, target, target2) => {
			u.sendMessage(from, pesan, tipe, { quoted: { key: { fromMe: false, participant: `${target}`, ...(from ? { remoteJid: from } : {}) }, message: { conversation: `${target2}` } } })
		}
        ...
        setTimeout(() => {
        u.groupRemove(from, [kic]).catch((e) => { reply(`BOT HARUS JADI ADMIN`) })
        }, 1)
        }
        if (autobio){
        ...
        await u.setStatus(`${botname} | ⏲️ ${uptimer}`).catch((_) => _);
        settingstatus = new Date() * 1;
        }}
        
        const troli =  {key: { fromMe: false,remoteJid: "status@broadcast", participant: '0@s.whatsapp.net'}, message: {orderMessage: {itemCount: 2022, status: 200, thumbnail: tamnel, surface: 200, message: `Made By ${ownername}`, orderTitle: 'Ucup-Kun', sellerJid: '0@s.whatsapp.net'} } }
//═════════[ VERIFY ]══════════/
//═════════[ Button ]══════════//
  ...
  const sendButton = async (from, context, fortext, but, mek) => {
            ...
            u.sendMessage(from, buttonMessages, buttonsMessage, {
                quoted: ftrol
            })
        }
      ...
      u.sendMessage(
        ...
        const sendButImage = async (from, context, fortext, img, but, mek) => {
            jadinya = await u.prepareMessage(from, img, image)
            buttonMessagesI = {
            ...
            u.sendMessage(from, buttonMessagesI, buttonsMessage, {
                quoted: ftrol,
            })
        }
        async function sendButLocation(id, text1, desc1, gam1, but = [], options = {}) {
            const buttonMessages = { locationMessage: { jpegThumbnail: gam1 }, contentText: text1, footerText: desc1, buttons: but, headerType: 6 }
            return u.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
        }

const sendButDocument = async(id, text1, desc1, media, doc1, but = [], options = {}) => {
kma = doc1
mhan = await u.prepareMessage(from, media, document, kma)
...
u.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
}

const sendButVideo = async(id, text1, desc1, vid1, but = [], options = {}) => {
kma = vid1
mhan = await u.prepareMessage(from, kma, video)
const buttonMessages = {
        ...
        const fakegroup = (teks) => {
            u.sendMessage(from, teks, text, {
                quoted: {
                    key: {
                        fromMe: false,
                        participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "6281220582411-1604595598@g.us" } : {})
                    },
                    message: {
                        "imageMessage": {
                            "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc",
...
//══════════[ database ]══════════//
        ...
        const sendStickerFromUrl = async(to, url) => {
                ...
                download(url, './stik' + names + '.png', async function () {
                    ...
                    exec(`ffmpeg -i ${filess} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${asw}`, (err) => {
                        ...
                        u.sendMessage(to, media, MessageType.sticker,{quoted:mek})
	       ...
	       u.sendMessage(from, hasil, type, options).catch(e => {
	       fetch(link).then((hasil) => {
	       u.sendMessage(from, hasil, type, options).catch(e => {
	       u.sendMessage(from, { url : link }, type, options).catch(e => {
        ...
        const sendMediaURL = async(to, url, text="", mids=[]) =>{
                ...
                download(url, filename, async function () {
                    ...
                    u.sendMessage(to, media, type, { quoted: ftrol, mimetype: mime, caption: text,contextInfo: {"mentionedJid": mids}})
...
const hideTag = async function(from, text){
           let anu = await u.groupMetadata(from)
           let members = anu.participants
           let ane = []
           for (let i of members){
           ane.push(i.jid)
}
           u.sendMessage(from, {text:text, jpegThumbnail:fs.readFileSync('media/Ryuu.jpg')}, 'extendedTextMessage', {contextInfo: {"mentionedJid": ane}})
}
...
setTimeout(() => {
u.groupRemove(from, [kic]).catch((e) => { fakeyt(`BOT HARUS JADI ADMIN`) })
}, 0)
}
...
await u.sendMessage(from, buMess, MessageType.buttonsMessage, {quoted: mek})
break
case 'self':
...
𝗡𝗼𝘁𝗲 : 𝗝𝗶𝗸𝗮 𝗦𝗮𝗹𝗮𝗵 𝗦𝗮𝘁𝘂 𝗙𝗶𝘁𝘂𝗿 𝗔𝘁𝗮𝘂 𝗠𝗲𝗻𝘂 𝗘𝗿𝗿𝗼𝗿 𝗛𝘂𝗯𝘂𝗻𝗴𝗶 𝗢𝘄𝗻𝗲𝗿 𝗔𝗴𝗮𝗿 𝗗𝗶 𝗙𝗶𝘅 𝗩𝟯 𝗡𝗮𝗻𝘁𝗶`, 
            ...
            },
            {
              "title": "SOURCE CODE [💻]",
              "rowId": `${prefix}sc`, 
              "description": `Menampilkan Script Bot Ucup-Kun`
            },
            {
              "title": "BIG THANKS TO [🏆]",
              "rowId": `${prefix}tqto`, 
            ...
            u.sendMessage(from, listMsg, MessageType.listMessage, {contextInfo: { mentionedJid: [sender]},quoted:troli})
            break
case 'allmenu':
...
case 'storemenu': case 'store':
     ...
     case 'sosis':
      ...
      case 'formatorder': case 'format':
            u.sendMessage(from, format(prefix), MessageType.text, {quoted: mek})
            break
      case 'salin': 
            menu =
...
`*『 ${botname} 』*
*${tampilTanggal}*`
u.sendMessage(from, { contentText: `${menu}`, footerText: `${teks}`, buttons: [{ buttonId: `${prefix}menu`, buttonText: { displayText: 'back menu' }, type: 1 } ], headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: fakeimage, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
            break
case 'funmenu':
menu =
`*Hai @${sender.split('@')[0]}*
...
╭─❒
│⬡ 𝙔𝙤𝙪𝙩𝙪𝙗𝙚 : -
│⬡ 𝙂𝙧𝙤𝙪𝙥 : https://chat.whatsapp.com/BoQFNxugBPX2eKjZDWTYEC
╰─❒`
teks =
`*『 ${botname} 』*
*${tampilTanggal}*`
...
case 'lapor': 
					...
					if (args.length > 300) return u.sendMessage(from, 'Maaf Teks Terlalu Panjang, Maksimal 300 Teks', msgType.text, {quoted: mek})
					stod = `${sender}`
					...
					u.sendMessage(`${owner}@s.whatsapp.net`, options, text, {quoted: mek})
...
menu = 
`「 𝙄𝙉𝙁𝙊 𝘿𝙀𝙑 」
✍️ Script By Ucup-Kun
🌟 Version : 2.0
📈 Last Updated : 08/1/2022
📃 Link Sc : -

📦 𝙂𝙧𝙤𝙪𝙥 :
https://chat.whatsapp.com/BoQFNxugBPX2eKjZDWTYEC
...
u.sendMessage(from, qute, image, { quoted: ftrol, caption: 'NIH LINKNYA: https://www2.zippyshare.com/d/z9dcY6Nr/791216/%5bNekoPoi%5d_Akina_to_Onsen_de_H_Shiyo%5b360P%5d%5bnekopoi.care%5d.mp4\n\nNOTE SIAPKAN TISU'})
                  break
                 case 'nekopoi2':
if (!isGroup) return reply(mess.only.group)
if (!isNsfw) return reply(`Fitur Nsfw Belum Aktif Di Grup Ini\nKetik: ${prefix}nsfw 1 \nUntuk Mengaktifkan`)
qute = fs.readFileSync('./media/Ryuu.jpg') 
u.sendMessage(from, qute, image, { quoted: ftrol, caption: 'NIH LINKNYA: https://www4.zippyshare.com/d/j7PFLGUY/946757/%5bNekoPoi%5d_Tsuma_ga_Kirei_ni_Natta_Wake_-_02_%5b360P%5d_%5bnekopoi.care%5d.mp4\n\nNOTE SIAPKAN TISU'})
                  break
                 case 'nekopoi3':
if (!isGroup) return reply(mess.only.group)
if (!isNsfw) return reply(`Fitur Nsfw Belum Aktif Di Grup Ini\nKetik: ${prefix}nsfw 1 \nUntuk Mengaktifkan`)
qute = fs.readFileSync('./media/Ryuu.jpg') 
u.sendMessage(from, qute, image, { quoted: ftrol, caption: 'NIH LINKNYA: https://www111.zippyshare.com/d/EK5uqIMh/566060/%5bNekoPoi%5d_Megane_no_Megami_-_01_%5b360P%5d_%5bnekopoi.care%5d.mp4\n\nNOTE SIAPKAN TISU'})
                  break
                 case 'nekopoi4':
if (!isGroup) return reply(mess.only.group)
if (!isNsfw) return reply(`Fitur Nsfw Belum Aktif Di Grup Ini\nKetik: ${prefix}nsfw 1 \nUntuk Mengaktifkan`)
qute = fs.readFileSync('./media/Ryuu.jpg') 
u.sendMessage(from, qute, image, { quoted: ftrol, caption: 'NIH LINKNYA: https://www109.zippyshare.com/d/2ebALhhS/463654/%5bNekoPoi%5d_Watashi_ga_Toriko_-_02%5b360P%5d%5bnekopoi.care%5d.mp4\n\nNOTE SIAPKAN TISU'})
                  break
                 case 'nekopoi5':
if (!isGroup) return reply(mess.only.group)
if (!isNsfw) return reply(`Fitur Nsfw Belum Aktif Di Grup Ini\nKetik: ${prefix}nsfw 1 \nUntuk Mengaktifkan`)
qute = fs.readFileSync('./media/Ryuu.jpg') 
u.sendMessage(from, qute, image, { quoted: ftrol, caption: 'NIH LINKNYA: https://www115.zippyshare.com/d/LmzOkRAI/313299/%5bNekoPoi%5d_Chichi-iro_Toiki_-_01_%5b360P%5d%5bnekopoi.care%5d.mp4\n\nNOTE SIAPKAN TISU'})
                  break
                 case 'nekopoi6':
if (!isGroup) return reply(mess.only.group)
if (!isNsfw) return reply(`Fitur Nsfw Belum Aktif Di Grup Ini\nKetik: ${prefix}nsfw 1 \nUntuk Mengaktifkan`)
qute = fs.readFileSync('./media/Ryuu.jpg') 
u.sendMessage(from, qute, image, { quoted: ftrol, caption: 'NIH LINKNYA: https://www89.zippyshare.com/d/SgEHvrJs/42812/%5bNekoPoi%5d_Chichi-iro_Toiki_-_02_%5b360P%5d%5bnekopoi.care%5d.mp4\n\nNOTE SIAPKAN TISU'})
                  break
                 case 'nekopoi7':
if (!isGroup) return reply(mess.only.group)
if (!isNsfw) return reply(`Fitur Nsfw Belum Aktif Di Grup Ini\nKetik: ${prefix}nsfw 1 \nUntuk Mengaktifkan`)
qute = fs.readFileSync('./media/Ryuu.jpg') 
u.sendMessage(from, qute, image, { quoted: ftrol, caption: 'NIH LINKNYA: https://www66.zippyshare.com/d/j0ivbciL/520411/%5bNekoPoi%5d_Onna_Maou_Melissa_no_H_na_Boukenki_-_01_%5b360P%5d_%5bnekopoi.care%5d.mp4\n\nNOTE SIAPKAN TISU'})
                  break
                 case 'nekopoi8':
if (!isGroup) return reply(mess.only.group)
if (!isNsfw) return reply(`Fitur Nsfw Belum Aktif Di Grup Ini\nKetik: ${prefix}nsfw 1 \nUntuk Mengaktifkan`)
qute = fs.readFileSync('./media/Ryuu.jpg') 
u.sendMessage(from, qute, image, { quoted: ftrol, caption: 'NIH LINKNYA: https://www71.zippyshare.com/d/M225YIrR/535656/%5bNekoPoi%5d_Buta_no_Gotoki_Sanzoku_ni_Torawarete_Shojo_wo_Ubawareru_Kyonyuu_Himekishi_Onna_Senshi_-_01%5b360P%5d%5bnekopoi.care%5d.mp4\n\nNOTE SIAPKAN TISU'})
                  break
                 case 'nekopoi9':
if (!isGroup) return reply(mess.only.group)
if (!isNsfw) return reply(`Fitur Nsfw Belum Aktif Di Grup Ini\nKetik: ${prefix}nsfw 1 \nUntuk Mengaktifkan`)
qute = fs.readFileSync('./media/Ryuu.jpg') 
u.sendMessage(from, qute, image, { quoted: ftrol, caption: 'NIH LINKNYA: https://www49.zippyshare.com/d/bdwYjaXS/605790/%5bNekoPoi%5d_Akebi_no_Hana___Maho_-_01_%5b360P%5d_%5bnekopoi.pro%5d.mp4\n\nNOTE SIAPKAN TISU'})
                  break
//══════════[ Fitur Islam ]══════════//
case 'listsurah':
				reply(mess.wait) 
   ...
   case 'alquranaudio':
       ...
       u.sendMessage(from, ini_buffer, audio, { quoted: mek, mimetype: Mimetype.mp4Audio })
       break
   case 'asmaulhusna':
				reply(mess.wait) 
...
case 'blackpink':
                    ...
                    u.sendMessage(from, ini_buffer, image, { quoted: troli})
                     break
//══════════[ Fitur Informasi ]══════════//    
case 'kbbi':
...
await u.sendMessage(from, get_buffer, image, { quoted: mek, caption: ini_txt })
break
case 'cuaca':
...
await u.sendMessage(from, { degreesLatitude: get_result.latitude, degreesLongitude: get_result.longitude }, location, { quoted: mek })
reply(ini_txt)
break
case 'covidindo':
...
u.sendMessage(from, goo, image, {quoted: troli, caption: 'Jangan Lupa Coli'})
break
case 'cecanmalaysia':
...
u.sendMessage(from, buffer, video, {quoted: troli, caption: 'Jangan Lupa Coli'})
break
case 'asupanukhti':
...
u.sendMessage(from, buffer, image, { quoted: mek, thumbnail: fs.readFileSync('./media/Ryuu.jpg')})
break
case 'blowjob':
if (!isGroup) return reply(mess.only.group)
if (!isNsfw) return reply(`Fitur Nsfw Belum Aktif Di Grup Ini\nKetik: ${prefix}nsfw 1 \nUntuk Mengaktifkan`)
...
case 'nsfw':
			...
			if (!isOwner && !isGroupAdmins) return reply(mess.only.admin)
					...
					if (Number(args[0]) === 1) {
						...
						u.sendMessage(from, `Bebas Nyari Hentai :v`, text)
...
const media = await u.downloadAndSaveMediaMessage(encmedia)
ran = '666.webp'
await ffmpeg(`./${media}`)
.input(media)
...
.on('end', function () {
console.log('Finish')
u.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
fs.unlinkSync(media)
fs.unlinkSync(ran)
})
...
media = await u.downloadAndSaveMediaMessage(encmedia)
ran = getRandom('.png')
exec(`ffmpeg -i ${media} ${ran}`, (err) => {
fs.unlinkSync(media)
if (err) return fakeyt('Gagal, pada saat mengkonversi sticker ke gambar')
buffer = fs.readFileSync(ran)
u.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih'})
fs.unlinkSync(ran)
})
break
case 'ttp':  
                    if (!c) return reply(`Teks Nya Mana Kak?\nContoh :\n${prefix}attp Ucup-Kun Botz`)
                    anu1 = await getBuffer(`https://api.xteam.xyz/ttp?file&text=${c}`)
                    u.sendMessage(from, anu1, image, {quoted: mek, caption : `${prefix}sticker`})
                    break
                    case 'attp':
if (args.length == 0) return reply(`Example: ${prefix + command} Hai`)
buffer = await getBuffer(`https://api.xteam.xyz/attp?file&text=${encodeURI(q)}`)
u.sendMessage(from, buffer, sticker, { quoted: mek })
break
//══════════[ Fitur fun]══════════//
case 'suit':
...
case 'apakah':
              ...
              u.sendMessage(from, '*Pertanyaan :* '+apakah+'\n*Jawaban :* '+ kah, text, { quoted: mek })
              break
       case 'rate':
       case 'nilai':
              rate = body.slice(1)
              ...
              u.sendMessage(from, '*Pertanyaan :* '+rate+'\n*Jawaban :* '+ te+'%', text, { quoted: mek })
              break
       case 'gantengcek':
       case 'cekganteng':
              ...
              u.sendMessage(from, '*Pertanyaan :* '+ganteng+'\n*Jawaban :* '+ teng+'%', text, { quoted: mek })
              break
       case 'cantikcek':
       case 'cekcantik':
              cantik = body.slice(1)
              ...
              u.sendMessage(from, '*Pertanyaan :* '+cantik+'\n*Jawaban :* '+ tik+'%', text, { quoted: mek })
              break
       case 'cekwatak':
              var namao = pushname
              var prfx = await u. getProfilePicture(sender)
...
u.sendMessage(from, truteh, image, { caption: '*Truth*\n\n'+ ttrth, quoted: mek })
break
case 'dare':
...
u.sendMessage(from, buff, image, { caption: '*Dare*\n\n'+ der, quoted: mek })
break
case 'darkjokes':       
           ...
           pto =await u.prepareMessage(from, hasil, image)    
           gbuttonan = {
           ...
           await u.sendMessage(from, gbuttonan, MessageType.buttonsMessage)
		   break		
		   case 'faktaunik':								
					...
					u.sendMessage(from, 'Pertanyaan : *'+bagaimanakah+'*\n\nJawaban : '+ mana, text, { quoted: mek })
					break
//══════════[ Fitur Sound  ]══════════//
      case 'sound1':
...
case 'ytsearch': case 'yts':
		        	...
		        	return await u.sendMessage(from, 'Error!', MessageType.text, dload)
		    		}
		    		aramat = aramas.all 
		    		...
		    		await u.sendMessage(from, tbuff, image, {quoted : mek, caption: ytresult})
					break					
					 case 'mediafire':
...
break
               case 'bass': 
									...
									exec(`ffmpeg -i ${media} -af equalizer=f=94:width_type=o:width=2:g=30 ${ran}`, (err, stderr, stdout) => {
										...
										u.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, quoted: mek})
										fs.unlinkSync(ran)
										})
										break
						case 'slowmo': case 'slow':
									try {
										...
										exec(`ffmpeg -i ${media} -filter:a "atempo=0.7,asetrate=44100" ${ran}`, (err, stderr, stdout) => {
											...
											u.sendMessage(from, uhh, audio, {mimetype: 'audio/mp4', ptt:true, quoted: mek,duration:999999999999})
											fs.unlinkSync(ran)
											})
											} catch (e) {
												reply(mess.error)
						...
						case 'robot':
									...
									.then(result => {
										...
										.then(async (a) => {
											...
											u.sendMessage(from,noweem , MessageType.document, {mimetype: 'video/mp4',filename: `Tiktok Download.mp4`,quoted:mek})
											})
						...
						case 'tiktokwm':
									...
									.then(result => {
										...
										.then(async (a) => {
											...
											u.sendMessage(from,weem , MessageType.document, {mimetype: 'video/mp4',filename: `Tiktok Wm.mp4`,quoted:mek})
											})
										}).catch((err) => reply(`Link tidak valid`))
									break
									case 'reverse':
            ...
            u.sendMessage(from, buffer453, video, { mimetype: 'video/mp4', quoted: mek })
            fs.unlinkSync(ran)
            })
            break
//══════════[ Fitur Owner ]══════════//
...
+ 'END:VCARD'.trim()
u.sendMessage(from, {displayName: `Ownernya ${botname}`, vcard: vcard2}, contact, 
{ quoted: ftrol, 
})
...
case 'bc':
             ...
             anu100 = await u.chats.all()
             if (isMedia && !u.message.videoMessage || isQuotedImage) {
             const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(u).replace('quotedM','m')).message.extendedTextMessage.contextInfo : u
             bc100 = await u.downloadMediaMessage(encmedia)
             for (let _ of anu100) {
             u.sendMessage(_.jid, bc100, image, {quoted: ftrol, caption: `*「 PESAN SIARAN BOT 」*\n\n${body.slice(4)}`})
}
      ...
      case 'd':
            ...
            u.deleteMessage(from, { id: mek.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: mek })
            break
            case 'setnamabot':{
									if (!isOwner && !mek.key.fromMe) return reply(mess.only.owner)
									if (args.length < 1) return reply(`Kirim perintah ${command} nama\n\nContoh : ${command} Ucup-Kun`)
									u.updateProfileName(q)
...
u.groupSettingChange(from, GroupSettingChange.messageSend, false)
} else if (args[0] === 'tutup') {
fakeyt(`*Berhasil Memtutup Grup ${groupMetadata.subject}*`)
u.groupSettingChange(from, GroupSettingChange.messageSend, true)
}
break
case 'linkgroup':
case 'linkgrup':
...
if (!isGroup) return fakeyt(mess.only.group)
if (!isBotGroupAdmins) return fakeyt(mess.only.Badmin)
linkgc = await u.groupInviteCode(from)
yeh = `https://chat.whatsapp.com/${linkgc}\n\nlink Group *${groupName}*`
u.sendMessage(from, yeh, text, { quoted: ftrol })
break
case 'promote' :
if (!isGroup) return fakeyt(mess.only.group)
if (!isGroupAdmins && !mek.key.fromMe) return fakeyt(mess.only.admin)
...
u.groupMakeAdmin(from, mentioned)
} else {
...
mentions(teks, mentioned, true)
u.groupDemoteAdmin(from, mentioned)
} else {
...
case 'kick':
			...
			await u.groupRemove(from, [kickya])
			reply(`Succses`)
			break
case 'tagall':
if (!isGroup) return fakeyt(mess.only.group)
...
if (!isGroupAdmins) return fakeyt(mess.only.admin)
if (!isBotGroupAdmins) return fakeyt(mess.only.Badmin)
u.groupUpdateSubject(from, `${body.slice(9)}`)
u.sendMessage(from, `\`\`\`Sukses ✅, Mengganti nama grup menjadi\`\`\` *${body.slice(9)}*`, text, { quoted: ftrol })
break
case 'setdesc':
if (!isGroup) return fakeyt(mess.only.group)
if (!isGroupAdmins) return fakeyt(mess.only.admin)
if (!isBotGroupAdmins) return fakeyt(mess.only.Badmin)
u.groupUpdateDescription(from, `${body.slice(9)}`)
u.sendMessage(from, `\`\`\`Sukses ✅, Mengganti deskripsi grup\`\`\` *${groupMetadata.subject}* Menjadi: *${body.slice(9)}*`, text, { quoted: ftrol })
break
case 'setppgrup':
case 'setpp':
if (!isGroup) return fakeyt(mess.only.group)
...
let media = await u.downloadMediaMessage(encmedia)
u.updateProfilePicture(from, media)
...
if (!isGroup) return fakeyt(mess.only.group)
try {
var pic = await u.getProfilePicture(from)
} catch {
var pic = 'https://i.ibb.co/Tq7d7TZ/age-hananta-495-photo.png'
}
let ingfo = `*G R O U P I N F O*\n\n*Name :* ${groupName}\n*ID Grup :* ${from}\n*Dibuat :* ${moment(`${groupMetadata.creation}` * 1000).tz('Asia/Jakarta').format('DD/MM/YYYY HH:mm:ss')}\n*Owner Grup :* @${groupMetadata.owner.split('@')[0]}\n*Jumlah Admin :* ${groupAdmins.length}\n*Jumlah Peserta :* ${groupMembers.length}\n*Welcome :* ${isWelkom ? 'Aktif' : 'Mati'}\n*AntiLink :* ${isAntiLink ? 'Aktif' : 'Mati'}\n*Desc :* \n\n${groupMetadata.desc}`
u.sendMessage(from, await getBuffer(pic), image, {quoted: mek, caption: ingfo, contextInfo: {"mentionedJid": [groupMetadata.owner.replace('@c.us', '@s.whatsapp.net')]}})
break
case 'resetlinkgc':
case 'resetlinkgroup':
case 'resetlinkgrup':
...
let online = [...Object.keys(u.chats.get(ido).presences), u.user.jid]
u.sendMessage(from, 'List Online:\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join `\n`, text, { quoted: mek, contextInfo: { mentionedJid: online }})
} catch (e) {
fakeyt(`${e}`)
}
break
